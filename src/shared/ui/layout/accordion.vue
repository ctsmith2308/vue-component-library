...
<template>
  <div class="accordion w-full space-y-2">
    ...
    >
      ...
      <Transition
        name="accordion"
        @enter="(el: Element) => { (el as HTMLElement).style.height = 'auto'; const height = (el as HTMLElement).offsetHeight; (el as HTMLElement).style.height = '0px'; setTimeout(() => { (el as HTMLElement).style.height = height + 'px'; }); }"
        @after-enter="(el: Element) => { (el as HTMLElement).style.height = 'auto'; }"
        @leave="(el: Element) => { (el as HTMLElement).style.height = (el as HTMLElement).offsetHeight + 'px'; setTimeout(() => { (el as HTMLElement).style.height = '0px'; }); }"
      >
        <div
          v-if="isActive(index)"
