...
const codeSnippet = `<Chip label="Vue 3" :removable="true" @remove="removeChip('Vue 3')" />`;
...
<template>
  <DocumentExampleSection :heading="heading" :description="htmlString" :code-snippet="codeSnippet">
    <div class="flex gap-2 flex-wrap justify-center min-h-8">
      <Chip v-for="chip in chips" :key="chip" :label="chip" color="primary" :removable="true" @remove="removeChip(chip)" />
